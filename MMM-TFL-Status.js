...
Module.register("MMM-TFL-Status", {
  defaults: {
    ...
    names: { "london-overground": "Overground", elizabeth: "Elizabeth Line" },
  },
  ...
  getDom: function () {
    ...
    var finishedLoading =
      Object.keys(this.responses).length == this.config.modes.length;
    var errors = [];
    ...
    if (startedLoading) {
          ...
          for (var i = 0; i < result.length; i++) {
            ...
            for (var j = 0; j < r.lineStatuses.length; j++) {
              if (r.lineStatuses[j].validityPeriods.length < 2) {
                ...
                for (
                  var k = 0;
                  k < r.lineStatuses[j].validityPeriods.length;
                  k++
                ) {
      ...
      linesTable.className = "tflStatus small";
      ...
      lines.forEach(function (line) {
        ...
        lineIndicator = document.createElement("td");
        lineIndicator.className = "lineIndicator " + line.id;
        ...
        lineRow.appendChild(lineIndicator);
        ...
        if (
          !that.config.ignore_good ||
          line.status != "Good Service" ||
          (that.config.lines_always_show !== null &&
            that.config.lines_always_show.includes(line.id))
        ) {
